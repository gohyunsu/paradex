...
## Purpose
Camera-to-robot (C2R) hand-eye calibration for xArm or Franka FR3 via Tsai-Lenz
AX=XB: drive arm through a trajectory observing a Charuco board, combine FK
motions with triangulated board motions.

## Files
- `capture.py` — Main PC. Builds the arm through `get_arm(args.arm)`; `remote_camera_controller("handeye_calibration")`. `save_current_camparam(<root>/0)`. Iterates `*_qpos` files from `get_handeye_calib_traj(arm)` (sorted by leading int): `controller.move(action, is_servo=False)`, `rcc.start("image", ...)/stop()`, save `robot.npy`, `eef.npy` (=robot_data["position"]), `qpos.npy` per idx. Supports `--arm xarm` and `--arm franka`.
- `calculate.py` — Main PC, runs at import (no `if __name__`). `undistort_and_detect_charuco` (ImageDict undistort → triangulate_charuco → `charuco_3d_{ids,corners}.npy` + detection overlay), `compute_fk` (RobotWrapper FK using `EEF_LINK`: xArm `link6`, Franka `fr3_link8` → `eef_fk.npy`), `compute_motion` (relative eef + camera motions; `SOLVE_XA_B` for cam point fit; floor board id range "1" excluded via `_floor_board_id_range`), `solve_ax_xb(motion_wrt_cam, motion_wrt_robot)` → `robot_wrt_cam`, save `C2R.npy`. `debug` overlays robot mesh + reprojected markers and prints FK/marker stats.

## paradex modules used
- `paradex.calibration.Tsai_Lenz.solve_ax_xb` — AX=XB hand-eye solve.
- `paradex.robot.robot_wrapper.RobotWrapper` + `paradex.robot.utils.get_robot_urdf_path` — FK on `link6` or `fr3_link8`.
...
- Franka calculate should be run without ROS on `PYTHONPATH` if ROS Pinocchio conflicts with conda Pinocchio: `PYTHONPATH= python src/calibration/handeye/calculate.py --arm franka`.
