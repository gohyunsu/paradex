...
| Python env | Paradex-compatible env with ROS 2 Python packages visible for teaching/replay/capture |
...
alias calc-env='PYTHONPATH= python'      # for calculate.py if ROS Pinocchio conflicts
```

If `rclpy` fails with `GLIBCXX_3.4.30` inside the conda env, run ROS Python commands
with:
...
If ROS's `pinocchio` conflicts with the conda/Paradex Pinocchio or NumPy stack,
clearing `PYTHONPATH` makes the project environment load its own package set.
...
|---|---|---|
| teaching / replay / capture | Paradex env with ROS 2 Python packages visible | **yes** (`ros-setting`) — needs rclpy |
| calculate | Paradex env | **no** (`calc-env` when needed) — needs project Pinocchio |
