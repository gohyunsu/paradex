...
### 6.3.1 실시간 multi-view UI

여러 카메라가 실제로 들어오는지 브라우저에서 보려면 main PC에서 아래를 실행한다.
...
브라우저에서 `http://<main-pc>:8792`를 열고 **Start stream**을 누른다.
이 버튼은 capture PC에 `stream_client.py`를 SSH로 띄우고,
`remote_camera_controller`로 acquisition을 arm한 뒤 stream sink를 켠다.
화면에는 병합된 저해상도 JPEG preview, capture PC 상태, camera frame id,
frame age가 표시된다.

주의할 점:

- 이 UI는 preview용이다. dataset 저장은 `image_remote.py`, `video_remote.py`,
  dataset acquisition script를 사용한다.
- 닫기 전 **Stop**을 눌러야 capture PC의 preview client가 `exit` 명령을 받고 종료된다.
- frame이 안 나오면 daemon 상태, `data_sender/main.py`, `remote_camera_controller.py`
  순서로 transport와 camera health를 확인한다.
...
- {doc}`Live Camera UI <live_camera_ui>`
